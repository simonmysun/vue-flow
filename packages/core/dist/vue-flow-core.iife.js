var VueFlowCore = function(exports, vue) {
  ...
  function rendererPointToPoint({ x, y }, { x: tx, y: ty, zoom: tScale }, ancestorZoom) {
    return {
      x: x * tScale * ancestorZoom + tx,
      y: y * tScale * ancestorZoom + ty
    };
  }
  function pointToRendererPoint({ x, y }, { x: tx, y: ty, zoom: tScale }, snapToGrid = false, snapGrid = [1, 1], ancestorZoom = 1) {
    const position = {
...
Edge: ${id2}`,
  ...
  function getHandleBounds(type, nodeElement, nodeBounds, zoom2, ancestorZoom, nodeId) {
    ...
    return Array.from(handles).map((handle) => {
      ...
      return {
        ...
        x: (handleBounds.left - nodeBounds.left) / zoom2 / ancestorZoom,
        y: (handleBounds.top - nodeBounds.top) / zoom2 / ancestorZoom,
  ...
  function useGetPointerPosition() {
    ...
    return (event) => {
      ...
      const pointerPos = pointToRendererPoint({ x, y }, viewport.value, void 0, void 0, ancestorZoom.value);
      ...
      if (isMouseTriggered && event.button === 0 || !isMouseTriggered) {
        let onPointerMove = function(event2) {
          ...
          const newConnection2 = {
            ...
            ...previousConnection,
            isValid,
            to: result.toHandle && isValid ? rendererPointToPoint({ x: result.toHandle.x, y: result.toHandle.y }, viewport.value, ancestorZoom.value) : connectionPosition,
      ...
      for (const element of updates) {
        ...
        if (node) {
          ...
          if (doUpdate) {
            ...
            node.handleBounds.source = getHandleBounds("source", update.nodeElement, nodeBounds, zoom2, state.ancestorZoom, node.id);
            node.handleBounds.target = getHandleBounds("target", update.nodeElement, nodeBounds, zoom2, state.ancestorZoom, node.id);
            changes.push({
              id: node.id,
              type: "dimensions",
              dimensions
  ...
  const _sfc_main$8 = /* @__PURE__ */ vue.defineComponent({
    ...
    setup(__props) {
      ...
      vue.onMounted(() => {
        ...
        let prevEventTransform = identity;
        vue.watch(
